...
% Author : Pavlos. Vrachnis , AM: 236010 , Date : 16/01/2020

% function: matrix2latex2(...)
% Author:   M. Koehler
...
% Enjoy life!!!
    ...
    Mstyle = []; %prosthetw Mstyle opws ta alla args
    rowLabels = [];
    colLabels = [];
    alignment = 'l';
    format = [];
   ...
   if(Mstyle == 'pmatrix')
       %arxizw to matrix
        fprintf(fid, '\\begin{equation*} Matrix= \\begin{pmatrix}\n');
        for i=1:height
             %gia kathe sthlh allazw grammh dhladh sto telos vazw \\
             if i>1
            fprintf(fid, '%s \\\\', matrix{i,j});
             end
            for j=1:width-1
                %gia kathe sthlh vazw &
                fprintf(fid, '%s&', matrix{i,j});
            end 
        end
        %vazw sto teleutaio row \\
        fprintf(fid, '%s \\\\', matrix{i,j});
        %termatizw to matrix
   ...
   elseif(Mstyle == 'vmatrix')
       %arxizw to matrix
        fprintf(fid, '\\begin{equation*} Matrix= \\begin{vmatrix}\n');
        for i=1:row
             %gia kathe sthlh allazw grammh dhladh sto telos vazw \\
             if i>1
            fprintf(fid, '%s \\\\', matrix{i,j});
            ...
            for j=1:col-1
                %gia kathe sthlh vazw & alla oxi sthn teleftaia
                fprintf(fid, '%s&', matrix{i,j});
            end 
        end
        %vazw sto teleutaio row \\
   ...
   elseif(Mstyle == 'bmatrix')
       %arxizw to matrix
        fprintf(fid, '\\begin{equation*} Matrix= \\begin{bmatrix}\n');
        for i=1:row
             %gia kathe sthlh allazw grammh dhladh sto telos vazw \\
             if i>1
            fprintf(fid, '%s \\\\', matrix{i,j});
